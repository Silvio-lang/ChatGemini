...
// A importação do pdf.js não é mais necessária, mas podemos deixar comentada para o futuro
...
// Contadores de métricas por sessão (zerados a cada recarga)
let requestCount = 0;
...
// VERSÃO SIMPLIFICADA DA FUNÇÃO DE ENVIO
...
function limparConversa() {
    if (confirm("Deseja realmente limpar a conversa?")) {
        ...
        document.getElementById("infoArquivoCarregado").textContent = ""; // CORREÇÃO ADICIONADA
    }
}
...
function salvarConversa() {
    ...
    document.getElementById("infoArquivoCarregado").textContent = `(Salvo: ${a.download})`; // CORREÇÃO ADICIONADA
}
...
document.getElementById("carregarJson").addEventListener("change", function () {
    ...
    document.getElementById("infoArquivoCarregado").textContent = `(Carregado: ${file.name})`; // CORREÇÃO ADICIONADA
...
async function iniciarTutorial() {
    ...
    limparConversa(); // Garante que a conversa e o status do arquivo sejam limpos
    const status = document.getElementById("status");
